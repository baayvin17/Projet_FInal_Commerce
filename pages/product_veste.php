...
<body>
    ...
    <div class="mt-5 container">
        ...
        if (isset($_GET['id'])) {
            ...
            $stmt = $conn->prepare("SELECT * FROM veste WHERE id_veste = ?");
            ...
            if ($result->num_rows > 0) {
                ...
                $avis_stmt = $conn->prepare("SELECT * FROM avis_veste WHERE id_produit = ?");
                $avis_stmt->bind_param("i", $product_id);
                $avis_stmt->execute();
                $avis_result = $avis_stmt->get_result();

                while ($avis = $avis_result->fetch_assoc()) {
                    echo "<p><strong>" . $avis['nom_utilisateur'] . " :</strong> " . $avis['commentaire'] . " (Note: " . $avis['note'] . "/5)</p>";
                }

                echo "<h3 class='mt-5'>Ajouter un avis</h3>";
                echo "<form action='ajouter_avis_veste.php' method='post'>";
